...
import { ArrowDown, ArrowRight, ArrowUpRight, Check, Cloud, Copy, ExternalLink, Github, Menu, X } from 'lucide-react';
...
function App() {
  ...
  return <>
    ...
    <main id="main">
      ...
      <section id="contact" className="contact-section"><div className="wrap contact-wrap"><div className="section-index">08 / NEXT STEPS</div><div className="contact-grid"><div><div className="eyebrow"><span className="live-dot" /> OPEN TO GRADUATE OPPORTUNITIES</div><h2>Good systems start<br />with a <span className="accent-text">conversation.</span></h2><p>Looking to grow in cloud and DevOps engineering. Based in Auckland and open to connecting with teams building thoughtful, reliable infrastructure.</p><div className="contact-buttons"><a className="button button-primary" href={`mailto:${email}`}>Email me <ArrowUpRight size={16} /></a><button className="button button-outline" onClick={copyEmail}>{copied ? <Check size={15} /> : <Copy size={15} />}{copied ? 'Copied to clipboard' : 'Copy email'}</button></div><a className="email-address" href={`mailto:${email}`}>{email}</a></div><div className="contact-side"><div className="contact-side-line"/><span className="section-index">BASED IN</span><strong>Auckland</strong><span>New Zealand <span className="meta-separator">·</span> NZST</span><a href="https://maps.google.com/?q=Auckland+New+Zealand" target="_blank" rel="noreferrer">47° 28′ S &nbsp; 167° 20′ E <ExternalLink size={12} /></a></div></div></div></section>
    </main>
    <footer className="footer"><div className="wrap footer-inner"><a className="brand" href="#top"><span className="brand-mark">P<span>.</span></span><span className="brand-name">PRASHANT SINGH</span></a><span className="footer-note">Built with intention. Auckland, New Zealand.</span><a href="#top" className="back-top">BACK TO TOP <ArrowUpRight size={13} /></a><span className="footer-copy">© {new Date().getFullYear()} PRASHANT SINGH</span></div></footer>
  </>;
}
